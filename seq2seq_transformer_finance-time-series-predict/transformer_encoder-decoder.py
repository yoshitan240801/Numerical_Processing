...
class MyTransformer(nn.Module):  # num_headが8だとGPUでOutOfMemoryになる
    def __init__(self, in_n, hidden_n, out_n, time_series, num_transformer_layer_encoder,
                 num_transformer_layer_decoder, num_head=5, dimension_feedforward=512, dropout_rate=0.2):
        super().__init__()
        ...
        transformer_layer_decoder = nn.TransformerDecoderLayer(d_model=hidden_n,
                                                                nhead=num_head,
                                                                dim_feedforward=dimension_feedforward,
                                                                dropout=dropout_rate,
                                                                activation="gelu",
        ...
        self.layer_2_decoder = nn.TransformerDecoder(decoder_layer=transformer_layer_decoder,
                                                     num_layers=num_transformer_layer_decoder,
                                                     norm=layer_normalization_decoder)
        self.layer_3_decoder = nn.Linear(hidden_n, out_n)
    ...
    def forward(self, input_encoder, input_decoder):
        ...
        out_layer_1_decoder = self.layer_1_decoder(input_decoder)
        out_layer_2_decoder = self.layer_2_decoder(tgt=out_layer_1_decoder,
                                                   memory=out_layer_3_encoder,
                                                   tgt_mask=None,
                                                   memory_mask=None,
                                                   tgt_key_padding_mask=None,
                                                   memory_key_padding_mask=None)
        output_decoder = self.layer_3_decoder(out_layer_2_decoder)
...
# DataFrameを1ミニバッチTensorに変換する関数　※推論で使用
...
number_transformer_layer_decoder = 2
...
nn_model = MyTransformer(in_n=input_number,
                         ...
                         num_transformer_layer_decoder=number_transformer_layer_decoder,
...
scheduler = optim.lr_scheduler.LinearLR(optimizer,
                                        start_factor=0.1,
                                        end_factor=0.01,
                                        total_iters=10)  # エポック毎に学習率を更新するschedulerを設定
...
for epoch in range(10):
    ...
    scheduler.step()  # 最適化関数の学習率の値をschedulerの設定を踏まえて更新
...
with torch.no_grad():
    ...
    while True:
        ...
        output = nn_model(input_encoder=test_explain_encoder_tensor_onebatch_after_ln,
                          input_decoder=test_explain_decoder_tensor_onebatch_after_ln)
